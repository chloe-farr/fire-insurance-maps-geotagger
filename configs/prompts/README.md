...
One prompt per file; `scripts/fim_hunyuan.py --preset NAME` and `scripts/fim_tile_ocr.py --preset NAME`
read `NAME.txt` from this directory. These are the exact prompts used in the runs under `runs/`
(recovered from `*_metadata.json`), so re-runs are comparable.
...
| `text_coords_tampa_<year>` (1884, 1887, 1889, 1892, 1895, 1899, 1915) | HunyuanOCR | same request for the Library of Congress Sanborn sheets of Tampa, one per edition year; `text_coords_tampa_1889` made `data/example/tampa_1889_sheet3/` |
